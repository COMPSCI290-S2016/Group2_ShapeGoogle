...
function SimplePCCanvas(glcanvas, shadersRelPath) {
    ...
    glcanvas.reRenderPointCloud = function(drawNormals) {
        glcanvas.drawer.reset();
        for (var i = 0; i < glcanvas.points.length; i++) {
            glcanvas.drawer.drawPoint(glcanvas.points[i], [1.0, 0.0, 0.0]);
            if (drawNormals) {
                var P1 = glcanvas.points[i];
                var P2 = vec3.create();
                vec3.scaleAndAdd(P2, P1, glcanvas.normals[i], glcanvas.NL);
                glcanvas.drawer.drawLine(P1, P2, [0.0, 0.0, 1.0]);
            }
        }
        requestAnimFrame(glcanvas.repaint);
    }
    
    glcanvas.loadPointCloud = function(points, normals, drawPCA) {
        glcanvas.points = points;
        glcanvas.normals = normals;
        //Center camera
        ...
        glcanvas.NL = bbox.getDiagLength()/15.0;//Normal length
